...
import IMG2 from "../components/Land/new2.png"
...
const Login = () => {
  ...
  const navigate = useNavigate();
    const { data, loading, error, AuthLogin } = useAuth();
  ...
  const filltestdetails = () => {
    console.log('clcied test pass')
    setseevalue(!seevalue);
    const emailInput = document.getElementById('email');
    const passwordInput = document.getElementById('password');
    
    console.log('seevaliue',seevalue)
    if (seevalue) {
      emailInput.value = 'testcred@gmail.com';
      passwordInput.value = 'TestCred';
      setFormdata({
            ...
            <form action className="sm:w-2/3 w-full px-4 lg:px-0 mx-auto" onSubmit={handleSubmit}>
              ...
              <div className="flex items-start">
                <div className="flex items-center h-5">
                  <input onClick={filltestdetails} id="viewpassword" name='viewpassword' aria-describedby="remember" type="checkbox" className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-primary-600 dark:ring-offset-gray-800" />
                </div>
                <div className="ml-3 text-sm">
                  <label htmlFor="remember" className="text-gray-500 dark:text-gray-300">Fill Test Details</label>
                </div>
              </div>
